...
class ImageSegmentationDatamodule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        val_images_dir = join(
            dirname(realpath(__file__)), "data", "validation", "images"
        )
        val_masks_dir = join(
            dirname(realpath(__file__)), "data", "validation", "images"
        )

        self.train_set = ImageSegmentationDataset(
            train_images_dir, train_masks_dir, ["pet"], self.train_transform
        )

        self.val_set = ImageSegmentationDataset(
            val_images_dir, val_masks_dir, ["pet"], self.val_transform
        )
